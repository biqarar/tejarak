...
class model extends \mvc\model
{
	...
	public function get_enter($_args)
	{
		...
		if($this->login())
		{
			...
			{
				$this->redirector(\lib\option::config('redirect'))->redirect();
				return;
			}
		}
	...
	public function set_main_session()
	{
		...
		$url = \lib\define::get_current_language_string(). '/'. \lib\option::config('redirect');
	...
	public function post_enter($_args)
	{
		...
		if($get_step)
		{
			...
			{
				$this->signup    = false;
				$this->user_data = \lib\db\users::get_by_username($mobile);
				if(
