...
trait get
{
	 ...
	 */
	public function ready_score($_data, $_options = [])
	{
		$default_options =
		[
	 ...
	 */
	public function get_list_score($_args = [])
	{
		if(!$this->user_id)
		{
			return false;
		...
		{
			logs::set('api:score:term:invalid:not:set', $this->user_id, $log_meta);
		...
		{
			logs::set('api:score:term:invalid:schoolid:not:set', $this->user_id, $log_meta);
		...
		$meta['school_userlessons.status']    = 'enable';
	 ...
	 * @return     <type>  The team.
	 */
	public function get_score($_options = [])
	{
		$default_options =
		[
			'debug' => true,
