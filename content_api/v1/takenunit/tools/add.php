...
trait add
{
	 ...
	 */
	public function takenunit($_args = [])
	{
		// default args
		$default_args =
		[
		...
		if(!$this->user_id)
		{
			logs::set('api:takenunit:user_id:notfound', $this->user_id, $log_meta);
		...
		if(!$school)
		{
			logs::set('api:takenunit:school:not:set', $this->user_id, $log_meta);
		...
		if(!in_array($type, ['takenunit', 'removeunit']))
		{
			logs::set('api:takenunit:type:invalid', $this->user_id, $log_meta);
		...
		if(!$lesson_id)
		{
			logs::set('api:takenunit:lesson_id:not:set', $this->user_id, $log_meta);
		...
		{
			logs::set('api:takenunit:lesson_detail:not:found', $this->user_id, $log_meta);
		...
		{
			logs::set('api:takenunit:lesson_detail:status:is:not:enable', $this->user_id, $log_meta);
