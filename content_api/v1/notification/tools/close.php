...
trait close
{
	 ...
	 */
	public function close_notification($_args = [])
	{
		if(!$this->user_id)
		{
			return false;
		...
		$notification_details = \lib\db\notifications::access_notification_id($id, $this->user_id, ['action' => 'close']);
		...
		if(\lib\db\notifications::update(['status' => 'close'], $notification_details['id']))
		{
			...
			debug::warn(T_("The notification was closed"));
		}
