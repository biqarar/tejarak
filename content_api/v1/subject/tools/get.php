...
trait get
{
	 ...
	 */
	public function ready_subject($_data, $_options = [])
	{
		$default_options =
		[
	 ...
	 */
	public function get_list_subject($_args = [])
	{
		if(!$this->user_id)
		{
			return false;
		...
		if(!$school_id || !is_numeric($school_id))
		{
			...
			logs::set('api:subject:get:list:school_id:invalid', $this->user_id);
		...
		{
			$check = $this->ready_subject($value);
			if($check)
			{
				$temp[] = $check;
			}
	 ...
	 * @return     <type>  The team.
	 */
	public function get_subject($_options = [])
	{
		$default_options =
		[
			'debug' => true,
		...
		$result = $this->ready_subject($result);
