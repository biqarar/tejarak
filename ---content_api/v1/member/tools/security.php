...
trait security
{
	...
	public function check_security($_team_id, $_args, $args, $log_meta)
	{
		...
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:team:notfound', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("Team not found"), 'user', 'permission');
			return false;
		}
		// load the logined user data
		$load_user = \dash\db\users::get(['id' => $this->user_id, 'limit' => 1]);
		if(!$load_user || !isset($load_user['id']))
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:user-id:notfound', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:user:can:not:edit:member', $this->user_id, $log_meta);
		...
		$change_id = \dash\utility::request('id');
		if(!$change_id)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:edit:id:not:set:member', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You can not edit member data"), 'user', 'permission');
			return false;
		}
		...
		if(!$change_id)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:id:invalid', $this->user_id, $log_meta);
		...
		{
			$change_creator = true;
			if(\dash\utility::isset_request('rule') && \dash\utility::request('rule') !== 'admin')
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:change:detail:creator:rule', $this->user_id, $log_meta);
				if($_args['debug']) \dash\notif::error(T_("Only creator of team can edit her security detail"), 'user', 'permission');
				return false;
			}

			if(\dash\utility::isset_request('status') && \dash\utility::request('status') !== 'active')
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:change:detail:creator:status:not:active', $this->user_id, $log_meta);
				if($_args['debug']) \dash\notif::error(T_("Only creator of team can edit her security detail"), 'status', 'permission');
				return false;
			}
			// the creator sended mobile
			if(\dash\utility::request('mobile') && $mobile = \dash\utility\filter::mobile(\dash\utility::request('mobile')))
			{
				...
				{
					if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:change:detail:creator:status:not:mobole', $this->user_id, $log_meta);
		...
		{
			if(\dash\utility::isset_request('rule') && \dash\utility::request('rule') !== 'user')
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:add:admin:rule:not:user1', $this->user_id, $log_meta);
		...
		{
			if(\dash\utility::isset_request('rule') && \dash\utility::request('rule') !== 'user' && !$change_creator)
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:add:admin:rule:not:user', $this->user_id, $log_meta);
				if($_args['debug']) \dash\notif::error(T_("Only creator of team can add or edit admins"), 'user', 'permission');
				return false;
			}
			...
			if(\dash\utility::isset_request('status') && \dash\utility::request('status') !== 'active')
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:add:admin:status:not:active', $this->user_id, $log_meta);
			...
			if(!$access)
			{
				if($_args['save_log']) \dash\db\logs::set('api:member:admin:can:not:edit:admin', $this->user_id, $log_meta);
		...
		if($is_creator)
		{
			...
			if(intval($this->user_id) === intval($change_id_in_team['user_id']))
			{
				if(\dash\utility::isset_request('rule') && \dash\utility::request('rule') !== 'admin')
				{
					if($_args['save_log']) \dash\db\logs::set('api:member:creator:can:not:set:her:as:user', $this->user_id, $log_meta);
				...
				if(\dash\utility::isset_request('status') && \dash\utility::request('status') !== 'active')
				{
					if($_args['save_log']) \dash\db\logs::set('api:member:creator:can:not:set:her:as:not:active', $this->user_id, $log_meta);
		...
		if(!$access)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:crator:can:not:edit:her:mobile', $this->user_id, $log_meta);
