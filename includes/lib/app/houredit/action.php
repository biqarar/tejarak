...
trait action
{
	 ...
	 */
	public static function hourrequest_action($_args = [])
	{
		...
		if(!\dash\user::id())
		{
			\dash\db\logs::set('api:houredit:action:user_id:not:set', \dash\user::id(), $log_meta);
		...
		if(!$id)
		{
			\dash\db\logs::set('api:houredit:action:id:not:set', \dash\user::id(), $log_meta);
		...
		if(!$type)
		{
			\dash\db\logs::set('api:houredit:action:type:not:set', \dash\user::id(), $log_meta);
		...
		{
			\dash\db\logs::set('api:houredit:action:type:inavalid', \dash\user::id(), $log_meta);
			\dash\notif::error(T_("Invalid action type"), 'type', 'arguments');
			return false;
		}

		$response = \dash\app::request('response');
		if($response && mb_strlen($response) > 500)
		{
			\dash\db\logs::set('api:houredit:action:response:max:length', \dash\user::id(), $log_meta);
		...
		if(!$team_id)
		{
			\dash\db\logs::set('api:houredit:action:team:id:not:set', \dash\user::id(), $log_meta);
			\dash\notif::error(T_("Team id not set"), 'team', 'arguments');
			return false;
		}

		// check this user is admin of this team
		$access_team = \lib\db\teams::access_team_id($team_id, \dash\user::id(), ['action' => 'admin']);

		// check the hour exist
		if(!isset($access_team['id']))
		{
			\dash\db\logs::set('api:houredit:action:access:forbidden', \dash\user::id(), $log_meta);
		...
		{
			\dash\db\logs::set('api:houredit:id:record:not:found', \dash\user::id(), $log_meta);
		...
		{
			\dash\db\logs::set('api:houredit:id:record:is:not:awaiting', \dash\user::id(), $log_meta);
		...
		{
			\dash\db\logs::set('api:houredit:team:id:hourrequests:team_id:not:mathc', \dash\user::id(), $log_meta);
		...
		{
			\dash\db\logs::set('api:houredit:action:access:forbidden:user:not:in:team', \dash\user::id(), $log_meta);
		...
		if($type === 'accept')
		{
			...
			if(isset($hourrequests_details['hour_id']) && $hourrequests_details['hour_id'] && is_numeric($hourrequests_details['hour_id']))
			{
				...
				{
					\dash\db\logs::set('api:houredit:action:hour_id:set:but:not:found', \dash\user::id(), $log_meta);
				...
				\dash\db\logs::set('api:hour:edited:request:accepted', \dash\user::id(), $log_meta);
			}
			else
			{
				// hour request have not hour id
				// need to add new record
				\lib\db\hours::record_process($hourrequests_details, null, ['type' => 'insert', 'user_id' => \dash\user::id()]);
				$meta['inserted_hour_id'] = \dash\db::insert_id();
				\dash\db\logs::set('api:hour:added:request:accepted', \dash\user::id(), $log_meta);
			}
		}
