...
trait last
{
	 ...
	 */
	public static function report_last_time()
	{
		if(!\dash\user::id())
		{
			return false;
		...
		if($user)
		{
			...
			if(!$user_id)
			{
				\dash\db\logs::set('api:report:user:id:set:but:is:not:valid', \dash\user::id(), $log_meta);
		...
		if($user_id)
		{
			if(!$check_is_my_team = \lib\db\teams::access_team_id($id, \dash\user::id(), ['action' => 'report_last_all']))
			{
		...
		else
		{
			...
			{
				\dash\db\logs::set('api:report:last:permission:denide', \dash\user::id(), $log_meta);
				\dash\notif::error(T_("Can not access to load detail of this team"), 'team', 'permission');
				return false;
			}
		}
		...
		$meta['order']      = 'DESC';
		$meta['pagenation'] = \dash\app::request('export') ? false : true;
		...
		{
			$check = self::ready_last_report($value);
			if($check)
			{
				$temp[] = $check;
			}
	 ...
	 */
	public static function ready_last_report($_data)
	{
		$temp = [];
