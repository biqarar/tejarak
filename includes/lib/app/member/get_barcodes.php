...
trait get_barcodes
{
	public static function get_barcodes(&$_data)
	{
		$multi = false;
		...
		$userteam_ids_decode = array_map(function($_a){return \dash\coding::decode($_a);}, $userteam_ids);
	 ...
	 */
	public static function check_barcode($_id)
	{
			// set the log meta
		$log_meta =
		[
		...
		$barcode1 = \dash\app::request("barcode1");
		if($barcode1 && mb_strlen($barcode1) > 100)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:barcode:max:limit:barcode1', \dash\user::id(), $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:barcode:min:limit:barcode1', \dash\user::id(), $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You must set barcode larger than 3 character"), 'barcode', 'arguments');
			return false;
		}

		$qrcode1 = \dash\app::request("qrcode1");
		if($qrcode1 && mb_strlen($qrcode1) > 100)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:qrcode:max:limit:qrcode1', \dash\user::id(), $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:qrcode:min:limit:qrcode1', \dash\user::id(), $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You must set qrcode larger than 3 character"), 'qrcode', 'arguments');
			return false;
		}

		$rfid1 = \dash\app::request("rfid1");
		if($rfid1 && mb_strlen($rfid1) > 100)
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:rfid:max:limit:rfid1', \dash\user::id(), $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('api:member:rfid:min:limit:rfid1', \dash\user::id(), $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You must set rfid larger than 3 character"), 'rfid', 'arguments');
			return false;
		}

		if(\dash\app::isset_request('barcode1'))
		{
			self::check_barcode_update($barcode1, $_id, 'barcode1');
		}

		if(\dash\app::isset_request('qrcode1'))
		{
			self::check_barcode_update($qrcode1, $_id, 'qrcode1');
		}

		if(\dash\app::isset_request('rfid1'))
		{
			self::check_barcode_update($rfid1, $_id, 'rfid1');
		}
	 ...
	 */
	public static function check_barcode_update($_barcode, $_id, $_title)
	{
		$get_code =
		[
			'type'    => $_title,
