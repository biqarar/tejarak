...
class staff {
	...
	public static function set_time($_args)
	{
		...
		elseif(array_key_exists('end', $check_date) && $check_date['end'] == null)
		{
			// set start time
			// $this->start = strtotime("$today ". $check_date['start']);
			$start_time = date("H:i");
			if(isset($check_date['start']))
			{
				$start_time = $check_date['start'];
			}
			$end_time = $time;
