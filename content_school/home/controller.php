...
class controller extends \content_school\main\controller
{
	...
	function _route()
	{
		...
		{
			\lib\router::set_controller("content_school\\dashboard\\controller");
			return;
		}
		...
		)
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\school\\controller");
				return;
			}
			else
			{
		...
		)
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\lesson\\controller");
				return;
			}
			else
			{
		...
		)
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\member\\controller");
				return;
			}
			else
			{
		...
		if(preg_match("/^([a-zA-Z0-9]+)\/plan$/", $url))
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\plan\\controller");
				return;
			}
			else
			{
		...
		if(preg_match("/^([a-zA-Z0-9]+)\/gateway(|\/list)$/", $url))
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\gateway\\controller");
				return;
			}
			else
			{
		...
		if(preg_match("/(\/report\/)|(\/report$)/", $url))
		{
			...
			\lib\router::set_controller("content_school\\report\\controller");
			return;
		}
		...
		{
			\lib\router::set_controller("content_school\\houredit\\controller");
			return;
		}
		...
		if(preg_match("/^([a-zA-Z0-9]+)\/sendnotify/", $url))
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\sendnotify\\controller");
				return;
			}
			else
			{
		...
		if(preg_match("/^([a-zA-Z0-9]+)\/option\/owner/", $url))
		{
			if($is_creator)
			{
				\lib\router::set_controller("content_school\\option\\owner\\controller");
				return;
			}
			else
			{
		...
		if(preg_match("/^([a-zA-Z0-9]+)\/option/", $url))
		{
			if($is_admin)
			{
				\lib\router::set_controller("content_school\\option\\controller");
				return;
			}
			else
			{
