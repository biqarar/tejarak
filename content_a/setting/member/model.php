...
class model extends \content_a\main\model
{
	...
	public function post_member($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change member"), 'member');
		...
		$new_member = utility::post('member');
		if(!$new_member)
		{
			debug::error(T_("Plese fill the mobile field"), 'member');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_member))
		{
			debug::error(T_("Invalid mobile number"), 'member');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'member');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_member']         = $this->login('id');
		$meta['new_member']         = $this->user_data['id'];
		// $meta['new_member_data'] = $this->user_data;
		$meta['new_member_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'member');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your membership transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
