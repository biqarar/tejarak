...
class model extends \content_a\main\model
{
	...
	public function post_delete($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change delete"), 'delete');
		...
		$new_delete = utility::post('delete');
		if(!$new_delete)
		{
			debug::error(T_("Plese fill the mobile field"), 'delete');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_delete))
		{
			debug::error(T_("Invalid mobile number"), 'delete');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'delete');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_delete']         = $this->login('id');
		$meta['new_delete']         = $this->user_data['id'];
		// $meta['new_delete_data'] = $this->user_data;
		$meta['new_delete_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'delete');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your deleteship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
