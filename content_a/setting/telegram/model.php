...
class model extends \content_a\main\model
{
	 ...
	 */
	public function post_telegram($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change telegram"), 'telegram');
		...
		$new_telegram = utility::post('telegram');
		if(!$new_telegram)
		{
			debug::error(T_("Plese fill the mobile field"), 'telegram');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_telegram))
		{
			debug::error(T_("Invalid mobile number"), 'telegram');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'telegram');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_telegram']         = $this->login('id');
		$meta['new_telegram']         = $this->user_data['id'];
		// $meta['new_telegram_data'] = $this->user_data;
		$meta['new_telegram_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'telegram');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your telegramship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
