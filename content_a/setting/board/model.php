...
class model extends \content_a\main\model
{
	 ...
	 */
	public function post_board($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change board"), 'board');
		...
		$new_board = utility::post('board');
		if(!$new_board)
		{
			debug::error(T_("Plese fill the mobile field"), 'board');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_board))
		{
			debug::error(T_("Invalid mobile number"), 'board');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'board');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_board']         = $this->login('id');
		$meta['new_board']         = $this->user_data['id'];
		// $meta['new_board_data'] = $this->user_data;
		$meta['new_board_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'board');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your boardship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
