...
class model extends \content_a\main\model
{
	 ...
	 */
	public function post_event($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change event"), 'event');
		...
		$new_event = utility::post('event');
		if(!$new_event)
		{
			debug::error(T_("Plese fill the mobile field"), 'event');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_event))
		{
			debug::error(T_("Invalid mobile number"), 'event');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'event');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_event']         = $this->login('id');
		$meta['new_event']         = $this->user_data['id'];
		// $meta['new_event_data'] = $this->user_data;
		$meta['new_event_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'event');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your eventship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
