...
class model extends \content_a\main\model
{
	...
	public function post_logo($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change logo"), 'logo');
		...
		$new_logo = utility::post('logo');
		if(!$new_logo)
		{
			debug::error(T_("Plese fill the mobile field"), 'logo');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_logo))
		{
			debug::error(T_("Invalid mobile number"), 'logo');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'logo');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_logo']         = $this->login('id');
		$meta['new_logo']         = $this->user_data['id'];
		// $meta['new_logo_data'] = $this->user_data;
		$meta['new_logo_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'logo');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your logoship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
