...
class model extends \content_a\main\model
{
	 ...
	 */
	public function post_sms($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change sms"), 'sms');
		...
		$new_sms = utility::post('sms');
		if(!$new_sms)
		{
			debug::error(T_("Plese fill the mobile field"), 'sms');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_sms))
		{
			debug::error(T_("Invalid mobile number"), 'sms');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'sms');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_sms']         = $this->login('id');
		$meta['new_sms']         = $this->user_data['id'];
		// $meta['new_sms_data'] = $this->user_data;
		$meta['new_sms_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'sms');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your smsship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
