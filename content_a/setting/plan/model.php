...
class model extends \content_a\main\model
{
	 ...
	 */
	public function post_plan($_args)
	{
		if(!$this->login())
		{
			debug::error(T_("Please login to change plan"), 'plan');
		...
		$new_plan = utility::post('plan');
		if(!$new_plan)
		{
			debug::error(T_("Plese fill the mobile field"), 'plan');
			return $this->refresh_page();
		}

		if(!$this->mobile = \lib\utility\filter::mobile($new_plan))
		{
			debug::error(T_("Invalid mobile number"), 'plan');
			return $this->refresh_page();
		}
		...
		{
			debug::error(T_("User not found"), 'plan');
			return $this->refresh_page();
		}
	...
	public function send_request()
	{
		...
		$meta['old_plan']         = $this->login('id');
		$meta['new_plan']         = $this->user_data['id'];
		// $meta['new_plan_data'] = $this->user_data;
		$meta['new_plan_mobile']  = $this->mobile;
		...
		{
			debug::error(T_("You try to move team to yourself!"), 'plan');
		...
		[
			...
			'needanswer'      => 1,
			'content'         => T_("The :alpha team has filed your planship transfer request, Do you accept this request?", ['alpha' => $this->team_detail['name']]),
		];
